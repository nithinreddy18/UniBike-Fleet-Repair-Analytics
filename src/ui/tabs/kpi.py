...
def get_kpi_data():
    ...
    try:
        ...
        if not work_orders_df.empty:
            ...
            open_df = work_orders_df[
                work_orders_df["status"].isin(["Open", "In Progress"])
            ]
            if not open_df.empty:
                open_df["created_at"] = pd.to_datetime(open_df["created_at"])
                now = pd.to_datetime("today")
                missed = open_df[
                    (now - open_df["created_at"]).dt.total_seconds() > 7 * 86400
                ]
        ...
        if not inventory_df.empty:
            fig_inv = px.bar(
                ...
                labels={"part_name": "Part", "quantity": "Quantity"},
            )
            # Add threshold line logic
            fig_inv.add_trace(
                go.Scatter(
                    x=inventory_df["part_name"],
                    y=inventory_df["threshold"],
                    mode="lines+markers",
                    name="Threshold",
        ...
        **Missed/Stalled Work Orders (> 7 days open):** {missed_count}
        ...
        return (
            ...
            fig_status,
            fig_inv,
            inventory_df[["part_name", "quantity", "threshold"]],
        )
...
def create_kpi_tab():
    with gr.Blocks() as tab:
        ...
        inventory_table = gr.Dataframe(
            headers=["Part Name", "Quantity", "Threshold"], interactive=False
        )
