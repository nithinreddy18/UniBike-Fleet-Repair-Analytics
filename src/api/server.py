...
def login(payload: LoginPayload):
    if payload.username == settings.gradio_admin_username and payload.password == settings.gradio_admin_password.get_secret_value():
...
def submit_ticket(payload: TicketPayload, db: Session = Depends(get_db)):
    ...
    if existing_ticket:
        raise HTTPException(status_code=400, detail=f"There is already an active ticket for '{payload.issue_type}' on Bike {payload.bike_id}.")
    ...
    db.commit()
    return {"message": "Success! Ticket submitted.", "bike_id": payload.bike_id, "status": "Out of Order"}
...
def get_tickets(db: Session = Depends(get_db)):
    ...
    return [{"id": t.id, "bike_id": t.bike_id, "issue_type": t.issue_type, "description": t.description, "status": t.status, "created_at": t.created_at} for t in tickets]

@app.patch("/api/tickets/{ticket_id}")
def update_ticket(ticket_id: int, payload: TicketUpdatePayload, db: Session = Depends(get_db)):
...
def get_kpi_data(db: Session = Depends(get_db)):
    ...
    if resolved_wos:
        ...
        if 'created_at' in resolved_df.columns and 'resolved_at' in resolved_df.columns:
            resolved_df['created_at'] = pd.to_datetime(resolved_df['created_at'], utc=True)
            resolved_df['resolved_at'] = pd.to_datetime(resolved_df['resolved_at'], utc=True)
    ...
    if open_wos:
        ...
        if 'created_at' in open_df.columns:
            ...
            missed_slas = len(open_df[(now - open_df['created_at']).dt.total_seconds() / 86400 > 2])
    ...
    inventory_data = [{"part_name": item.part_name, "quantity": item.quantity, "reorder_level": item.threshold} for item in inventory]
